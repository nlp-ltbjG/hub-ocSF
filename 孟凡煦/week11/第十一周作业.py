...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather。你可以一次调用多个工具，也可以根据工具返回结果再次调用工具。"
)


def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    循环调用：提问 → 模型输出 tool_call → 执行 → 回填 → 循环直到模型不再调用工具 → 最终回答。
    ...
    while iteration < max_iterations:
        ...
        # 模型没有输出 tool_calls，说明不需要再调用工具，生成最终回答
        if not msg.tool_calls:
            if verbose:
                print(f"  模型未请求工具调用，生成最终回答")
            break
    ...
    if verbose:
        print(f"\n  → [llm] 最终回答（共{iteration}轮迭代，{elapsed:.1f}s）")
