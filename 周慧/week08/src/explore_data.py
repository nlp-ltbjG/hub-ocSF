...
def _get_font():
    ...
    if _CN_FONT is None:
        ...
        for p in fm.findSystemFonts():
            p_lower = p.lower()
            # 排除不支持中文的 Noto Sans 变体
            if "noto" in p_lower and any(
                x in p_lower for x in ["adlam", "mro", "rejang", "sundanese", "bamum"]
            ):
                continue
            if any(
                k in p_lower
...
def plot_label_distribution(splits_data, output_dir):
    ...
    for ax, (split_name, rows) in zip(axes, splits_data.items()):
        ...
        bars = ax.bar(
            ["不相似 (0)", "相似 (1)"], counts, color=["#F44336", "#2196F3"], width=0.5
        )
...
def plot_char_length(rows, output_dir):
    ...
    print(f"  DEBUG: pos_lens 数量={len(pos_lens)}, neg_lens 数量={len(neg_lens)}")

    fp = _get_font()
    fig, ax = plt.subplots(figsize=(8, 4))

    # 限制 x 轴范围到 99 百分位，避免异常值导致图表失真
    x_max = np.percentile(all_lens, 99)
...
def plot_token_length(rows, tokenizer, output_dir):
    ...
    print(f"  图表已保存 → {save_path}")
    print(
        f"  Token 长度：均值={np.mean(token_lens):.1f}  "
...
def plot_length_diff(rows, output_dir):
    ...
    """
    pos_diffs = [
        abs(len(r["sentence1"]) - len(r["sentence2"])) for r in rows if r["label"] == 1
    ]
    neg_diffs = [
        abs(len(r["sentence1"]) - len(r["sentence2"])) for r in rows if r["label"] == 0
    ]

    # 限制 x 轴范围到 99 百分位，避免异常值导致图表失真
    all_diffs = pos_diffs + neg_diffs
    x_max = np.percentile(all_diffs, 99)
    ...
    ax.hist(
        ...
        bins=30,
        alpha=0.6,
        label=f"正样本 均值={np.mean(pos_diffs):.1f}",
        color="#2196F3",
        density=True,
        range=(0, x_max),
    )
    ax.hist(
        ...
        bins=30,
        alpha=0.6,
        label=f"负样本 均值={np.mean(neg_diffs):.1f}",
        color="#F44336",
        density=True,
        range=(0, x_max),
    )
    ...
    print(f"  图表已保存 → {save_path}")
    print(
        f"  长度差：正样本均值={np.mean(pos_diffs):.1f}  负样本均值={np.mean(neg_diffs):.1f}"
    )
...
def print_stats(name, rows):
    ...
    if n_unlabeled:
        ...
        print(
            f"  标签未公开（CLUE 竞赛格式）: {n_unlabeled:>6,} 条  —— 仅供参考，不用于评估"
        )
    else:
    ...
    print(
        f"  句子字符长度 — 均值={np.mean(all_lens):.1f}  中位数={np.median(all_lens):.0f}  "
        f"P95={np.percentile(all_lens, 95):.0f}  最长={max(all_lens)}"
    )
    print(f"  示例正样本：")
...
def parse_args():
    ...
    parser.add_argument(
        "--skip_token", action="store_true", help="跳过 Token 长度分析（较慢）"
    )
