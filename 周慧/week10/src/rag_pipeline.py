...
class VectorStore:
    def __init__(self, embed_model: object):
        import faiss
    ...
    def search(
        ...
        for score, idx in zip(scores[0], indices[0]):
            ...
            if filter_meta:
                if not all(
                    str(item.get(k, "")) == str(v) for k, v in filter_meta.items()
                ):
                    continue
...
def rerank(query: str, candidates: list[dict], top_k: int = TOP_K_RERANK) -> list[dict]:
    ...
    try:
        ...
        model_name = (
            str(model_path) if model_path.exists() else "BAAI/bge-reranker-base"
        )
        ...
        logger.warning(
            "sentence-transformers 未安装，跳过 Rerank（pip install sentence-transformers）"
        )
...
def rewrite_query(query: str, client: OpenAI) -> str:
    ...
    使用 deepseek-v4-flash（最快最便宜），不需要高质量推理。
    """
...
def build_context(retrieved: list[dict]) -> tuple[str, list[dict]]:
    ...
    for i, item in enumerate(retrieved, 1):
        ...
        citations.append(
            {"index": i, "source": label, "chunk_id": item.get("chunk_id", "")}
        )
...
class RAGPipeline:
    ...
    def query(
        ...
        # ① 查询改写（可选）
        retrieval_query = (
            rewrite_query(question, self.client) if self.use_qr else question
        )

        # ② 向量检索
        vec_results = self.vec_store.search(
            retrieval_query, TOP_K_RETRIEVE, filter_meta
        )
        if verbose:
            logger.info(
                f"向量召回: {len(vec_results)} 条，最高分={vec_results[0]['vec_score']:.3f}"
                if vec_results
                else "向量召回: 0 条"
            )
        ...
        if self.use_bm25 and self.bm25_store:
            ...
            if verbose:
                logger.info(
                    f"BM25 召回: {len(bm25_results)} 条，RRF 后: {len(candidates)} 条"
                )
        else:
...
def main():
    ...
    parser.add_argument(
        "--query-rewrite", action="store_true", help="开启查询改写（增加一次 LLM 调用）"
    )
    parser.add_argument(
        "--no-bm25", action="store_true", help="关闭 BM25（消融实验用）"
    )
    parser.add_argument(
        "--no-rerank", action="store_true", help="关闭 Rerank（消融实验用）"
    )
