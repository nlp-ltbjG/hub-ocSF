...
def chunk_semantic(
    ...
    def flush(buf: list[dict]) -> dict | None:
        ...
        meta = {
            ...
            "section": " > ".join(buf[0]["section_path"])
            if buf[0]["section_path"]
            else "",
...
def process_file(parsed_path: Path, strategy: str = STRATEGY):
    ...
    blocks = [{**b, "content": clean_text(b["content"])} for b in blocks]
    ...
    if strategy == "fixed":
        ...
        for chunk in chunk_hierarchical(blocks):
            ...
            if "parent_content" in chunk["metadata"]:
                chunk["metadata"]["parent_content"] = clean_text(
                    chunk["metadata"]["parent_content"]
                )
            raw_chunks.append(chunk)
...
def main():
    ...
    table_count = sum(
        1 for c in all_chunks if "table" in c["metadata"].get("block_types", [])
    )
