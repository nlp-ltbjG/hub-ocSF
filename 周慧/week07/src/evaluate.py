            ...
            for i in range(len(input_ids)):
                ...
                for j, gold_id in enumerate(token_labels):
                    ...
                    if use_crf:
                        pred_seq.append(
                            id2label.get(
                                pred_ids_list[i][j] if j < len(pred_ids_list[i]) else 0,
                                "O",
                            )
                        )
                    else:
...
def main():
    ...
    print(
        f"加载 checkpoint（epoch={ckpt['epoch']}，val_f1={ckpt['val_entity_f1']:.4f}）"
    )
    ...
    print("\n" + "=" * 70)
    print(
        f"模型：{'BERT + CRF' if args.use_crf else 'BERT + Linear'}  |  评估集：{split_name}"
    )
    print("=" * 70)
    ...
    if args.use_crf:
        if illegal_stats["total_illegal"] == 0:
            ...
            print(
                f"  → 提示：训练 epoch 不足时转移矩阵尚未收敛；充分训练（3+ epochs）后可降至 0"
            )
    else:
