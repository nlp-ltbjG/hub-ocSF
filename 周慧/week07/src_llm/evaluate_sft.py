...
def load_model(model_path: str, ckpt_dir: str, device: torch.device):
    ...
    if is_lora:
        if not PEFT_AVAILABLE:
            raise ImportError(
                "加载 LoRA adapter 需要 peft 库：pip install peft>=0.14.0"
            )
...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    """提取 gold spans，格式 (text, type, start, end)。支持 peoples_daily BIO 格式。"""
    spans = set()
    ...
    for i, (token, tag) in enumerate(zip(tokens, tags)):
        if tag.startswith("B-"):
            if current_entity:
                spans.add(("".join(current_text), current_entity, start_idx, i))
    ...
    if current_entity:
        spans.add(("".join(current_text), current_entity, start_idx, len(tokens)))

    return spans


def pred_spans_from_output(
    text: str, raw_output: str
...
def main():
    ...
    for i, record in enumerate(samples, 1):
        # peoples_daily 格式：tokens 数组，需要拼接成文本
        ...
        print(
            f"[{i:3d}/{len(samples)}] {status}  gold:{gold_str or '无'}  |  {text[:30]}"
        )
