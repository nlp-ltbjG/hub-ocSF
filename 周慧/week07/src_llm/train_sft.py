...
def record_to_target(record: dict) -> str:
    ...
    输入：{"tokens": ["我", "在", "北", "京"], "ner_tags": ["O", "O", "B-LOC", "I-LOC"]}
    输出：'{"entities": [{"text": "北京", "type": "LOC"}]}'
    """
...
class SFTDataset(Dataset):
    """
    把 peoples_daily NER 数据（Bio 格式）转换为 chat-format SFT 训练样本。
    ...
    Loss mask 结构：
      ...
      │ {"entities": [{"text": "北京", "type": "LOC"}]} <EOS>        │  → 真实 id
      └──────────────────────────────────────────────────────────────┘
    """
...
def main():
    ...
    print(
        f"\n训练配置：batch={args.batch_size}, grad_accum={args.grad_accum}, epochs={args.epochs}, lr={args.lr}"
    )
