...
class Agent:
    ...
    def _truncate_history(self):
        ...
        user_questions = [
            ...
            if m["role"] == "user"
            and not m.get("content", "").startswith("Observation:")
        ]
        if len(user_questions) > self.max_history_turns:
            start_idx = -1
    ...
    def ask(self, question: str, max_steps: int = 10) -> Generator[dict, None, None]:
        ...
        for step in range(1, max_steps + 1):
            ...
            if tool_fn is None:
                observation = (
                    f"未知工具 '{tool_name}'，可用工具：{list(self.TOOLS_MAP.keys())}"
                )
            else:
                try:
...
def run(
    question: str, max_steps: int = 10, verbose: bool = True
