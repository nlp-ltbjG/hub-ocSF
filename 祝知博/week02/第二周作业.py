...
基于pytorch框架编写模型训练
实现一个多分类任务的训练:一个随机向量，哪一维数字最大就属于第几类
...
class TorchModel(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.activation = nn.Softmax(dim=1)  # softmax激活函数，将输出转换为概率分布
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            y_pred = self.activation(x)  # 应用softmax获取概率分布
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            predicted_class = torch.argmax(y_p).item()  # 获取预测类别
            true_class = int(y_t.item())  # 获取真实类别
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        ...
        max_prob = float(torch.max(res).item())  # 获取最大概率值
        print("输入：%s, 预测类别：%d, 最大概率值：%f" % (vec, predicted_class, max_prob))  # 打印结果
