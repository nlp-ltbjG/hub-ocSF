...
EMBED_DIM       = 1024
LLM_MODEL       = "qwen-plus"              # DashScope 默认模型
DEEPSEEK_MODEL  = "deepseek-v4-flash"      # DeepSeek 默认模型（更快更便宜）
# DEEPSEEK_MODEL  = "deepseek-v4-pro"      # 更强但更贵
...
def get_client(llm_provider="dashscope") -> OpenAI:
...
def get_bge_embedding_model():
    """加载本地 BGE 模型进行 embedding（无需 API Key）"""
    if not BGE_MODEL_PATH.exists():
        raise EnvironmentError(
            f"本地 BGE 模型不存在: {BGE_MODEL_PATH}\n"
            "请先运行: python src_langchain/download_model.py\n"
            "或设置镜像: export HF_ENDPOINT=https://hf-mirror.com"
        )
    from sentence_transformers import SentenceTransformer
    logger.info(f"加载本地 BGE 模型: {BGE_MODEL_PATH}")
    return SentenceTransformer(str(BGE_MODEL_PATH))


# ── 向量检索 ──────────────────────────────────────────────────────────────────

class VectorStore:
    def __init__(self, client: OpenAI = None, bge_model=None):
        import faiss
        self.client    = client
    ...
    def _embed_query(self, query: str) -> np.ndarray:
        if self.bge_model is not None:
            vec = self.bge_model.encode(query, normalize_embeddings=True)
...
def rewrite_query(query: str, client: OpenAI, model: str = LLM_MODEL) -> str:
    """
...
def call_llm(query: str, context: str, client: OpenAI, model: str = LLM_MODEL) -> str:
    user_msg = (
        f"【参考资料】\n{context}\n\n"
        f"【问题】\n{query}\n\n"
...
class RAGPipeline:
    def __init__(
        ...
        embedding_mode:  str = "dashscope",
    ):
        self.client        = get_client(llm_provider)
        self.llm_provider  = llm_provider
        self.llm_model     = DEEPSEEK_MODEL if llm_provider == "deepseek" else LLM_MODEL
        self.embedding_mode = embedding_mode
        
        if embedding_mode == "bge":
            self.bge_model = get_bge_embedding_model()
            self.vec_store = VectorStore(bge_model=self.bge_model)
        else:
            self.bge_model = None
    ...
    def query(
        ...
        # ① 查询改写（可选）
        retrieval_query = rewrite_query(question, self.client, self.llm_model) if self.use_qr else question
        ...
        answer = call_llm(question, context, self.client, self.llm_model)
