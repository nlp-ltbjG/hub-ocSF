...
def embed_with_bge(model, texts: list[str], show_progress: bool = True) -> np.ndarray:
    """使用本地 BGE 模型计算 embedding"""
    from tqdm import tqdm
    embeddings = []
    iterable = tqdm(texts, desc="计算 Embedding") if show_progress else texts
...
def build_faiss_index(chunks: list[dict], client=None, embedding_mode="dashscope"):
    """
    构建 FAISS 向量索引。

    参数：
        chunks: 分块数据列表
        client: OpenAI 客户端（当 embedding_mode="dashscope" 时需要）
        embedding_mode: "dashscope" 使用 DashScope API，"bge" 使用本地 BGE 模型
    ...
    if embedding_mode == "bge":
        bge_model = get_bge_embedding_model()
        embeddings = embed_with_bge(bge_model, texts)
    else:
        if client is None:
            client = get_client()
        embeddings = embed_texts(client, texts)

    logger.info(f"构建 FAISS 索引，维度={embeddings.shape[1]}...")
