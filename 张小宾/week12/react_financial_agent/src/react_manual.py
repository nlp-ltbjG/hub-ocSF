...
def run(question: str, max_steps: int = 10, verbose: bool = True, history: list[dict] | None = None) -> Generator[dict, None, None]:
    """
    ...
    Args:
        ...
        max_steps: 最大步骤数
        verbose: 详细模式
        history: 历史对话列表，每个元素为 {"question": str, "answer": str}
    ...
    # 注入历史上下文（以 user/assistant 对话对形式）
    if history:
        for turn in history:
