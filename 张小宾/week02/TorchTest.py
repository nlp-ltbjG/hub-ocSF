...
# 随机生成一个5维向量，最大值的坐标就是类别
def build_sample():
    x = np.random.random(5)
    y = int(np.argmax(x))   # 最大值所在下标
    return x, y


# 随机生成一批样本
# 于多分类，CrossEntropyLoss 要求：
# X: FloatTensor
# Y: LongTensor
# Y 的 shape 是 (batch_size,)
# 标签值是类别编号，比如 0~4
...
def predict(model_path, input_vec):
    ...
    for vec, cls, prob_vec in zip(input_vec, pred, probs):
        cls = int(cls)
        print("输入：%s, 预测类别：%d, 该类别概率：%f" % (
            vec,
            cls,
            float(prob_vec[cls])
        ))
