...
FIG_DIR = ROOT / "peoples_outputs" / "figures"
...
def collect_stats(records: list) -> dict:
    ...
    if records and "tokens" in records[0]:
        data_format = "peoples_daily"
    else:
        data_format = "cluener"

    for row in records:
        if data_format == "peoples_daily":
            ...
            while i < len(ner_tags):
                ...
                if tag.startswith("B-"):
                    ...
                    while end + 1 < len(ner_tags) and ner_tags[end + 1] == f"I-{etype}":
                        end += 1
                    entity_type_counts[etype] += 1
                    entity_lengths.append(end - i + 1)
                    total_entities += 1
                    if etype not in entities_by_type:
                        entities_by_type[etype] = []
                    entities_by_type[etype].append("".join(tokens[i:end+1]))
                    i = end + 1
                else:
                    i += 1
        else:
...
def plot_entity_distribution(stats_train: dict):
    ...
    ax.set_title("各类实体频次分布（训练集）", fontsize=14)
    ax.set_ylabel("实体数量")
    ax.set_xlabel("实体类型")
    plt.tight_layout()
