...
LOG_DIR = ROOT / "peoples_outputs" / "logs"
...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    """提取 gold spans，格式：{(text, type, start, end)}。支持 cluener 和 peoples_daily 格式。"""
    spans = set()
    if "tokens" in record:
        # peoples_daily 格式
        ...
        while i < len(ner_tags):
            ...
            if tag.startswith("B-"):
                ...
                surface = "".join(tokens[i:end+1])
                spans.add((surface, etype, i, end))
                i = end + 1
            else:
                i += 1
    else:
