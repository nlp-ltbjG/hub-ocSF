...
# 根据数据集名称动态设置输出路径
def get_output_dirs(data_dir):
    """根据data_dir获取输出目录，如 data/lcqmc -> outputs/lcqmc/"""
    data_name = Path(data_dir).name
    output_dir = ROOT / "outputs" / data_name
    return output_dir / "checkpoints", output_dir / "logs"

#BERT_PATH  = ROOT.parent.parent / "pretrain_models" / "bert-base-chinese"
OUTPUT_DIR = ROOT / "outputs"/ "lcqmc"
