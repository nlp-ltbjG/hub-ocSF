...
# ========== 修改：从 tokens + ner_tags 提取 gold spans ==========
...
def main():
    ...
    # ── 输出结果 ──────────────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"LLM SFT NER 评估结果（样本数: {len(samples)}）")
    print(f"{'='*60}")
    print(f"{'精确率 (P)':<15} {metrics['precision']:.4f}")
    print(f"{'召回率 (R)':<15} {metrics['recall']:.4f}")
    print(f"{'F1 分数':<15} {metrics['f1']:.4f}")
    print(f"{'解析失败':<15} {parse_fail} 条")
    print(f"{'耗时':<15} {elapsed:.1f}s")
