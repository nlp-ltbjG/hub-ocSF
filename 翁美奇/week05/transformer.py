...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # torch.tril 是 PyTorch 中用于提取矩阵下三角
        causal_mask = torch.tril(torch.ones(T, T)).view(1, 1, T, T)  # (1,1,T,T)
        atten = atten.masked_fill(causal_mask == 0, float('-inf'))
        
        out = torch.softmax(atten, dim=-1) @ v 
...
class TransformerEncoder(nn.Module):
    def __init__(self, hidden, num_layers, nhead, dim_feedforward, dropout, max_len=512):
        super().__init__()
        self.layers = nn.ModuleList([
            EncoderLayer(hidden, nhead, dim_feedforward, dropout) for _ in range(num_layers)
        ])
        self.pos_embedding = nn.Embedding(max_len, hidden)   # 可学习位置编码

    def forward(self, x):
        B, T, H = x.shape
        positions = torch.arange(T).unsqueeze(0)  # (1, T)
        x = x + self.pos_embedding(positions)   # 加上位置编码
